...
def audit(transcript,visual=None):
    ...
    return {"schema_version":1,"measurements":measurements,"heuristic_judgments":judgments,"suggestions":suggestions,"preserve_mode_edl_unchanged":True,"forbidden_automatic_deletions":["caveats","debugging","conclusions"]}
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("--transcript",required=True);p.add_argument("--visual-analysis");p.add_argument("--out",required=True);a=p.parse_args();t=json.loads(Path(a.transcript).read_text(encoding="utf-8"));v=json.loads(Path(a.visual_analysis).read_text(encoding="utf-8")) if a.visual_analysis else None;r=audit(t,v);out=Path(a.out);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(r,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(out);return 0
