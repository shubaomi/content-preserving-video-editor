...
def build(title,transcript,terms):
    ...
    return {"schema_version":1,"recommended_platform":"both","douyin":douyin,"wechat_channels":wechat,"search_terms":terms}
def main():
    p=argparse.ArgumentParser(description=__doc__);p.add_argument("--title",required=True);p.add_argument("--transcript",required=True);p.add_argument("--term",action="append",default=[]);p.add_argument("--out",required=True);a=p.parse_args();r=build(a.title,json.loads(Path(a.transcript).read_text(encoding="utf-8")),a.term);out=Path(a.out);out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(r,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(out);return 0
